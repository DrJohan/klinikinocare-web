...
const content = {
  en: {
    ...
    nav: { home: 'Home', biowound: 'BioWound', cellmax: 'Cellmax', treatments: 'Care pathways', why: 'Why Inocare', journey: 'What to expect', faq: 'FAQs', visit: 'Visit us', training: 'Training', open: 'Open menu', close: 'Close menu' },
    ...
    treatments: {
      eyebrow: 'Three care pathways', title: 'Start with the concern that matters to you', intro: 'These pathways are informational. A clinician must assess you before discussing a suitable treatment plan.', ask: 'Ask about', learnWound: 'Explore BioWound',
      items: [
    ...
    why: {
      ...
      name: 'Dr Ahmad Nabil Fikri bin Norhashimi',
      qualification: 'Doctor of Medicine (MD), First Moscow State Medical University (Sechenov), Russia',
      registration: 'MMC Full Registration No. 105388',
      experienceLabel: 'Relevant clinical background',
      experience: [
        'Orthopaedic medical officer experience at Hospital Shah Alam, including acute and chronic wound management.',
        'Primary-care experience at Klinik Kesihatan Bagan Datuk, including responsibility for its wound-care team.',
        'Clinical skills listed include modern wound-care dressing, minor surgical procedures and rehabilitation support.',
      ],
      note: 'Professional background is summarised from the practitioner\'s resume. Care recommendations remain subject to individual clinical assessment.'
    },
    journey: {
  ...
  ms: {
    ...
    nav: { home: 'Utama', biowound: 'BioWound', cellmax: 'Cellmax', treatments: 'Laluan penjagaan', why: 'Mengapa Inocare', journey: 'Apa yang dijangka', faq: 'Soalan lazim', visit: 'Kunjungi kami', training: 'Latihan', open: 'Buka menu', close: 'Tutup menu' },
    ...
    treatments: {
      eyebrow: 'Tiga laluan penjagaan', title: 'Mulakan dengan masalah yang penting bagi anda', intro: 'Maklumat ini ialah panduan umum. Doktor perlu menilai anda sebelum membincangkan pelan rawatan yang sesuai.', ask: 'Tanya tentang', learnWound: 'Lihat BioWound',
      items: [
    ...
    why: {
      ...
      role: 'Pengamal perubatan berdaftar',
      name: 'Dr Ahmad Nabil Fikri bin Norhashimi',
      qualification: 'Doktor Perubatan (MD), First Moscow State Medical University (Sechenov), Rusia',
      registration: 'No. Pendaftaran Penuh MMC 105388',
      experienceLabel: 'Latar belakang klinikal berkaitan',
      experience: [
        'Pengalaman sebagai pegawai perubatan ortopedik di Hospital Shah Alam, termasuk pengurusan luka akut dan kronik.',
        'Pengalaman penjagaan primer di Klinik Kesihatan Bagan Datuk, termasuk tanggungjawab untuk pasukan penjagaan luka.',
        'Kemahiran klinikal yang disenaraikan termasuk pembalutan luka moden, prosedur pembedahan kecil dan sokongan rehabilitasi.',
      ],
      note: 'Latar belakang profesional diringkaskan daripada resume pengamal. Cadangan penjagaan kekal tertakluk pada penilaian klinikal individu.'
    },
    journey: {
...
function Nav({ language }: { language: Locale }) {
  ...
  const links = [[c.nav.home, '#home'], [c.nav.biowound, '/biowound/'], [c.nav.cellmax, '#cellmax'], [c.nav.treatments, '#treatments'], [c.nav.why, '#why'], [c.nav.journey, '#journey'], [c.nav.faq, '#faq'], [c.nav.visit, '#visit']];
  return <>
    <nav className="fixed inset-x-0 top-0 z-50 border-b border-[#DCEAF7]/70 bg-white/90 px-4 py-2 backdrop-blur-xl md:px-6" aria-label={c.a11y.navigation}><div className="mx-auto flex max-w-[1600px] items-center justify-between">
      ...
      <div className="hidden items-center gap-3 md:flex lg:gap-5"><a href="/biowound/" className="flex min-h-11 items-center rounded-full bg-[#DCEAF7] px-4 text-sm font-bold text-[#234173]">{c.nav.biowound}</a><a href="#treatments" className="hidden py-3 text-sm font-semibold text-[#234173] xl:block">{c.nav.treatments}</a><a href="#why" className="hidden py-3 text-sm font-semibold text-[#234173] lg:block">{c.nav.why}</a><a href="#visit" className="hidden py-3 text-sm font-semibold text-[#234173] lg:block">{c.nav.visit}</a>
    ...
    <div className={`fixed inset-0 z-40 overflow-hidden md:hidden ${open ? 'pointer-events-auto' : 'pointer-events-none'}`} aria-hidden={!open}><button type="button" aria-label={c.nav.close} className={`absolute inset-0 bg-[#1E293B]/30 ${open ? 'opacity-100' : 'opacity-0'}`} onClick={() => setOpen(false)}/><aside id="mobile-menu" className={`absolute right-0 h-full w-[88%] max-w-sm overflow-y-auto bg-white px-7 pb-8 pt-24 shadow-2xl transition-transform duration-500 ${open ? 'translate-x-0' : 'translate-x-full'}`}>
